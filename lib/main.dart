...
class _MyAppState extends State<MyApp> {
  ...
  late final DummySongCatalogRepository songRepository =
  DummySongCatalogRepository();
  ...
  Future<void> _bootstrap() async {
    try {
      ...
      final deps =
      AppDependencies(songCatalogRepositoryOverride: songRepository);
  ...
  Future<void> _refreshTodayStatus() async {
    if (!mounted) return;
    setState(() => _todayStatusLoading = true);
  ...
  void _switchTab(int index) {
    ...
    if (index == 0) _refreshTodayStatus();
  }
    ...
    return ValueListenableBuilder<AppThemeState>(
      ...
      builder: (context, themeState, _) {
        ...
        return MaterialApp(
          ...
          home: ready
              ? Builder(builder: (ctx) => _buildHomeScaffold(ctx, deps))
              : const Scaffold(
